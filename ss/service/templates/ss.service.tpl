...
ExecStart=/usr/bin/docker run --rm --name ss-{{service_port}} -v /home/core/ss/config-{{service_port}}.json:/tmp/config.json -v /var/log:/var/log --publish {{service_port}}:{{service_port}} a3linux/goss -c /tmp/config.json
